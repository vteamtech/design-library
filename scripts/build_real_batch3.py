#!/usr/bin/env python3
"""Build batch 3: 16 more real-world brand editions (80 pages)."""
import json, time, subprocess

PROFILES = json.load(open('/tmp/real_design_profiles_batch3.json'))
...
def make_hero(p):
    children = [
        ...
        {"type": "container", "settings": {"flex_direction": "row", "gap": px(16), "align_items": "center"},
         "children": [
            {"type": "widget", "widget_type": "html", "settings": {
                "html": f"<a href='/contact' style='display:inline-block;padding:{p['btn_padding']}px 36px;background:{p['accent']};color:{p['bg'] if p['bg'] not in ['#FFFFFF','#FAF9F6','#FDFCF8'] else '#000000'};text-decoration:none;font-weight:700;font-family:{p['heading_font']},sans-serif;border-radius:{p['btn_radius']}px;font-size:15px;'>Get Started</a>"}},
            {"type": "widget", "widget_type": "html", "settings": {
...
def make_features(p):
    ...
    for num, title, desc in items:
        ...
        cards.append({"type": "container", "settings": cs, "children": [
            ...
            {"type": "widget", "widget_type": "html", "settings": {"html": s(title, p['text'], 16, '700', p['heading_font'])}},
...
def make_cta(p):
    btn_text_color = p['dark_bg'] if p['accent'] not in ['#9FE870','#FFD02F','#C2EF4E'] else '#000000'
    return [{"type": "container", "settings": {
        ...
        "children": [
            ...
            {"type": "widget", "widget_type": "html", "settings": {"html": f"<div style='text-align:center;'><a href='/contact' style='display:inline-block;padding:{p['btn_padding']}px 40px;background:{p['accent']};color:{btn_text_color};text-decoration:none;font-weight:700;font-family:{p['heading_font']},sans-serif;border-radius:{p['btn_radius']}px;font-size:15px;'>Contact Us Today</a></div>"}}
    ]}]
...
def make_testimonial(p):
    return [{"type": "container", "settings": {
        ...
        "children": [
            ...
            {"type": "widget", "widget_type": "html", "settings": {"html": s("ALEXANDRA REID, FOUNDER & CEO", p['accent'], 13, '600', p['body_font'], 'none', 1, 'center')}}
    ]}]
...
def make_footer(p):
    ...
    for title, content in col_data:
        cols.append({"type": "container", "settings": {"flex_direction": "column", "width": {"unit": "%", "size": 25}, "gap": px(12)},
            "children": [
                {"type": "widget", "widget_type": "html", "settings": {"html": s(title, p['accent'], 12, '700', p['heading_font'], 'uppercase', 1)}},
...
for phil in list(PROFILES.keys()):
    p = PROFILES[phil]
    print(f"\n{'='*50}")
...
with open('/tmp/real_design_batch3_results.json', 'w') as f:
